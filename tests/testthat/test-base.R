...
test_that("base baselines", {
  ...
  expect_doppelganger("lines-accent", function() {
    plot(1:10, (1:10)^2, type = "b", pch = 19, xlab = "x", ylab = "y", col = thematic_current("accent"))
  })
  ...
  expect_doppelganger("smooth-accent", function() {
    ...
    lines(stats::lowess(cars), col = thematic_current("accent"))
  })
  ...
  expect_doppelganger("barplot-accent", function() {
    barplot(VADeaths[1:3, "Rural Male"], col = thematic_current("accent"))
  })
  ...
  expect_doppelganger("stripchart-cols", function() {
    stripchart(len ~ dose, data = ToothGrowth,
               pch = 19, col = thematic_current("qualitative"),
               method = "jitter")
  })
  ...
  expect_doppelganger("pies", function() {
    ...
    pie(df$value, labels = df$group, radius = 1, col = thematic_current("qualitative"))
  })

  expect_doppelganger("qq", function() {
    stats::qqnorm(ToothGrowth$len, pch = 1, frame = FALSE)
    stats::qqline(ToothGrowth$len, col = thematic_current("accent"), lwd = 2)
  })
  ...
  expect_doppelganger(
    ...
    function() {
      pairs(
        ...
        col = thematic_current("qualitative")[iris$Species],
        lower.panel = NULL
      )
    }
  )
