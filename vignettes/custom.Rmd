...
Complete **ggplot2** themes are `theme()` objects that fully specify every possible `theme()` element. The default **ggplot2** theme, `theme_gray()`, is a complete theme, and **ggplot2** provides some other useful complete themes such as `theme_bw()`, `theme_minimal()`, and `theme_classic()`. As we saw in the auto theming article, **thematic** can play nicely with complete themes __so long as they are set globally via `theme_set()`__. This is because **thematic**'s philosophy is to only ever impact global defaults (i.e., **thematic** updates colors and fonts in the global **ggplot2** theme), and never impact plot-specific code (which should always take precedence over the global defaults).

```{r}
# Sets theme_classic() as the global ggplot2 theme just for this plot
