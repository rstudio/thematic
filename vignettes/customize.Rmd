...
my_theme <- theme(
  ...
  strip.text = element_text(colour = thematic_get_mixture(1))
 )
p + my_theme
```
...
For each relevant `Geom` (e.g., `GeomPoint`), new `Geom$default_aes` defaults are set (based on `bg`, `fg`, `accent`, and `font`). Grayscale colours (e.g., `GeomPoint$default_aes$colour`) are assigned a mixture of `bg` and `fg` whereas non-grayscale colours (e.g., `GeomSmooth$default_aes$colour`) are assigned the `accent` colour. It's important to note these are just global defaults that only take effect if the aesthetic hasn't been specified:
...
`thematic_on()` sets a new `scale_[fill/colour]_continuous()` default (via `sequential`). See the previous section for an extensive discussion on how `sequential` works and note that  adding a relevant (continuous) scale renders `sequential` irrelevant:
...
4. Defines a new `scale_[fill/colour]_discrete()` default (via `qualitative`). As with `sequential` colorscales, adding a relevant (discrete) scale renders `qualitative` irrelevant:
...
Similar to **lattice**, **base** R graphics doesn't have a global distinction between a `qualitative` and `sequential` colourscales, it just has `palette()` (which is closest, semantically, to `qualitative`):
