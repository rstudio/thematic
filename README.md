...
1. Auto theming works in **rmarkdown** with `runtime: shiny` and also in `html_document_base()` based output formats with a non-`NULL` `theme` (i.e., powered by **bootstraplib**). With time, hopefully most other **rmarkdown** output formats will use `auto_preferences_set()` (internally) so that `'auto'` values can work the way you'd expect them to (at least by default). Note also that `accent='auto'` and `font='auto'` currently doesn't work with custom RStudio themes.
